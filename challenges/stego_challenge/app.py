...
@app.get("/download")
def download():
    if not os.path.exists("hidden.png"): abort(404, "instructor: add hidden.png to this folder")
...
def submit():
    ...
    if candidate == "hidden-string": return jsonify({"status":"correct","flag":decrypt_flag()})
