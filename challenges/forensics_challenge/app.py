...
@app.get("/download")
def download():
    if not os.path.exists("hidden.pcap"): abort(404, "instructor: add hidden.pcap to this folder")
...
def submit():
    ...
    if candidate == "extract-me": return jsonify({"status":"correct","flag":decrypt_flag()})
